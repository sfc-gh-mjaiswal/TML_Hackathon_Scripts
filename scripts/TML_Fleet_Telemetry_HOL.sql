...
-- Create your HOL sandbox database & schemas (all lab objects go here)
...
-- ============================================================
-- Import the shared data as a read-only database
-- ============================================================
-- The provider account locator is YG36360 (SFSEAPAC.TML_HACKATHON_ACCOUNT)

CREATE OR REPLACE DATABASE TML_FLEET_TELEMETRY FROM SHARE SFSEAPAC.TML_HACKATHON_ACCOUNT.TML_HACKATHON_SHARE;

-- Verify the import (all 4 schemas should be visible):
SHOW SCHEMAS IN DATABASE TML_FLEET_TELEMETRY;
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.RAW.VEHICLE_TELEMETRY_RAW_V1;         -- ~1.6M
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.MASTER.GEOFENCE_DEFINITIONS;          -- 25
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.MASTER.ROAD_SEGMENTS;                 -- 236
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.MASTER.CUSTOMER_RETENTION_POLICIES;   -- 201
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.CURATED.GPS_SIGNALS_V1;              -- ~1.6M
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.CURATED.J1939_SIGNALS_V1;            -- ~1.6M
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.BUSINESSREADY.TRIP_SUMMARIES_V1;     -- ~2,746
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.BUSINESSREADY.FLEET_HEALTH_DASHBOARD_V1; -- ~1,753
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.BUSINESSREADY.DIM_VEHICLE_V1;        -- ~1,753
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.BUSINESSREADY.DIM_CUSTOMER_V1;       -- 201
SELECT COUNT(*) FROM TML_FLEET_TELEMETRY.BUSINESSREADY.REALTIME_FLEET_STATUS; -- ~4.97M
